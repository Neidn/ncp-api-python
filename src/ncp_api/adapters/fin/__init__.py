...
class FinAdapter(NcpHttpAdapter):
    ...
    def __init__(self, env_base_url: str, signer: HmacSigner) -> None:
        ...
        self.global_dns = GlobalDnsApi(GLOBAL_DNS_BASE_URLS[NcpEnv.FIN], signer)
        ...
        self.object_storage = ObjectStorageApi(
            OBJECT_STORAGE_BASE_URLS[NcpEnv.FIN], signer
        )
        ...
        self.cloud_insight = CloudInsightApi(
            CLOUD_INSIGHT_BASE_URLS[NcpEnv.FIN], signer
        )
        ...
        self.cdss = CdssApi(CDSS_BASE_URLS[NcpEnv.FIN], signer)
        self.search_engine = SearchEngineApi(
            SEARCH_ENGINE_BASE_URLS[NcpEnv.FIN], signer
        )
        ...
        self.source_commit = SourceCommitApi(
            SOURCE_COMMIT_BASE_URLS[NcpEnv.FIN], signer
        )
        self.source_build = SourceBuildApi(SOURCE_BUILD_BASE_URLS[NcpEnv.FIN], signer)
        self.source_pipeline = SourcePipelineApi(
            SOURCE_PIPELINE_BASE_URLS[NcpEnv.FIN], signer
        )
        self.vpc_source_pipeline = SourcePipelineApi(
            VPC_SOURCE_PIPELINE_BASE_URLS[NcpEnv.FIN], signer
        )
        self.vpc_source_deploy = VpcSourceDeployApi(
            VPC_SOURCE_DEPLOY_BASE_URLS[NcpEnv.FIN], signer
        )
        self.resource_manager = ResourceManagerApi(
            RESOURCE_MANAGER_BASE_URLS[NcpEnv.FIN], signer
        )
        ...
        self.sens = SensApi(SENS_BASE_URLS[NcpEnv.FIN], signer)
        self.cloud_outbound_mailer = CloudOutboundMailerApi(
            CLOUD_OUTBOUND_MAILER_BASE_URLS[NcpEnv.FIN], signer
        )
