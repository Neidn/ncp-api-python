...
class GovAdapter(NcpHttpAdapter):
    ...
    def __init__(self, env_base_url: str, signer: HmacSigner) -> None:
        ...
        self.global_dns = GlobalDnsApi(GLOBAL_DNS_BASE_URLS[NcpEnv.GOV], signer)
        ...
        self.object_storage = ObjectStorageApi(
            OBJECT_STORAGE_BASE_URLS[NcpEnv.GOV], signer
        )
        ...
        self.nks = NksApi(NKS_BASE_URLS[NcpEnv.GOV], signer)
        # New adapters
        ...
        self.cdss = CdssApi(CDSS_BASE_URLS[NcpEnv.GOV], signer)
        self.search_engine = SearchEngineApi(
            SEARCH_ENGINE_BASE_URLS[NcpEnv.GOV], signer
        )
        ...
        self.source_commit = SourceCommitApi(
            SOURCE_COMMIT_BASE_URLS[NcpEnv.GOV], signer
        )
        self.source_build = SourceBuildApi(SOURCE_BUILD_BASE_URLS[NcpEnv.GOV], signer)
        self.source_pipeline = SourcePipelineApi(
            SOURCE_PIPELINE_BASE_URLS[NcpEnv.GOV], signer
        )
        self.vpc_source_pipeline = SourcePipelineApi(
            VPC_SOURCE_PIPELINE_BASE_URLS[NcpEnv.GOV], signer
        )
